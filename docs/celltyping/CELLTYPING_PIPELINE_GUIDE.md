...
**Module:** `src/spatialcore/annotation/`

---

## Overview

The SpatialCore cell typing pipeline provides end-to-end cell type annotation for spatial transcriptomics data. It trains custom CellTypist models on scRNA-seq references, produces calibrated confidence scores via z-score transformation, and maps predictions to Cell Ontology (CL) IDs.

**Key Value Proposition:**

| Approach | Gene Utilization | Confidence Calibration |
|----------|------------------|------------------------|
| Pre-trained models (Immune_All_High, etc.) | ~8% gene overlap with 400-gene panel | Raw logistic regression scores |
| **SpatialCore custom models** | **~100% gene overlap** | **Z-score transformed, interpretable [0,1]** |
...
│                                                                                  │
│  ╔══════════════════════════════════════════════════════════════════════════╗   │
│  ║  PHASE 1: DATA ACQUISITION (Run Once)                                     ║   │
│  ║  ─────────────────────────────────────                                    ║   │
│  ║  acquire_reference() → Download from source → Store to destination        ║   │
│  ║                                                                           ║   │
│  ║  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────────┐   ║   │
│  ║  │  CellxGene      │    │   Synapse       │    │   OUTPUT            │   ║   │
│  ║  │  cellxgene://   │───▶│   synapse://    │───▶│   Local / GCS / S3  │   ║   │
│  ║  └─────────────────┘    └─────────────────┘    └─────────────────────┘   ║   │
│  ╚══════════════════════════════════════════════════════════════════════════╝   │
│                                     │                                            │
│                                     ▼ (stored paths)                             │
...
│  ║                                                                           ║   │
│  ║  ┌────────────────┐              train_and_annotate()                     ║   │
│  ║  │ SPATIAL DATA   │     |        OR low-level functions:                  ║   │
│  ║  │ (Xenium,CosMx) │     │                                                 ║   │
│  ║  └────────────────┘     │        1. get_panel_genes()                     ║   │
│  ║          │              │        2. combine_references()                  ║   │
│  ║          ▼              │        3. add_ontology_ids()                    ║   │
│  ║  get_panel_genes()      │        4. subsample_balanced()                  ║   │
│  ║          │              │        5. train_celltypist_model()              ║   │
│  ║          ▼              ▼        6. annotate_celltypist()                 ║   │
│  ║  ┌────────────────────────┐      7. filter_low_confidence()               ║   │
│  ║  │ combine_references()   │      8. add_ontology_ids() (to predictions)   ║   │
│  ║  │ ├─ Load (backed mode)  │                                               ║   │
│  ║  │ ├─ Ensembl→HUGO        │                                               ║   │
│  ║  │ ├─ log1p(10k)          │                                               ║   │
│  ║  │ └─ Gene intersection   │                                               ║   │
...
│  ║          ▼                                                                ║   │
│  ║  ┌────────────────────────┐      ┌─────────────────────────────────┐      ║   │
│  ║  │ train_celltypist_     │─────▶│ annotate_celltypist()            │      ║   │
│  ║  │ model()               │      │ ├─ Z-score confidence            │      ║   │
...
│  ║                                                                           ║   │
│  ║  ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐              ║   │
│  ║  │ DEG Heatmap     │ │ 2D Validation   │ │ Confidence Plot │              ║   │
│  ║  │ (top N genes)   │ │ (GMM-3)         │ │ (spatial+jitter)│              ║   │
│  ║  └─────────────────┘ └─────────────────┘ └─────────────────┘              ║   │
│  ║                                                                           ║   │
│  ║  ┌─────────────────┐                                                      ║   │
│  ║  │ Ontology Table  │                                                      ║   │
...
All SpatialCore outputs use the [CellxGene schema](https://github.com/chanzuckerberg/single-cell-curation/blob/main/schema/3.0.0/schema.md) for interoperability:
...
│  ─────────                        ────────────                              │
│  • cellxgene://dataset_key        • Local: /data/refs/lung.h5ad             │
│  • cellxgene://?tissue=lung       • GCS:   gs://bucket/refs/lung.h5ad       │
│  • synapse://syn12345678          • S3:    s3://bucket/refs/lung.h5ad       │
│                                                                              │
│                    acquire_reference()                                       │
│  Source ──────────────────────────────────────────────▶ Destination         │
│                                                                              │
│  • Downloads data from public database                                       │
│  • Stores to local or cloud storage                                         │
│  • Returns path for use in Phase 2                                          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
s3_path = acquire_reference(
    ...
    max_cells=100000,  # Limit for memory
)

# Download from Synapse
path = acquire_reference(
    source="synapse://syn12345678",
    output="/data/references/synapse_ref.h5ad",
)
```
...
# View available predefined datasets
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PHASE 2: TRAINING & ANNOTATION WORKFLOW                                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 1: get_panel_genes()                                              │ │
│  │ Extract gene names from spatial data var_names                         │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 2: combine_references()                                           │ │
│  │ ├─ Resolve URIs (local, gs://, s3://)                                   │ │
│  │ ├─ Load via backed mode (memory efficient)                              │ │
│  │ ├─ Stratified subsample per reference (max_cells_per_ref)               │ │
│  │ ├─ Gene normalization: Ensembl → HUGO                                   │ │
│  │ ├─ Expression normalization: log1p(10k)                                 │ │
│  │ ├─ Intersect with panel genes                                           │ │
│  │ └─ Track source in .obs["reference_source"]                             │ │
│  │                                                                          │ │
│  │ OUTPUT: .obs["original_label"], .obs["reference_source"]                │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 3: add_ontology_ids(skip_if_exists=True)                          │ │
│  │ ├─ Fill missing CL IDs via 4-tier matching                              │ │
│  │ ├─ Preserve existing CellxGene native IDs                               │ │
│  │ └─ Enable semantic grouping for balancing                               │ │
│  │                                                                          │ │
│  │ OUTPUT: .obs["cell_type_ontology_term_id"]                              │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 4: subsample_balanced(group_by_column="cell_type_ontology_       │ │
│  │                              term_id")                                   │ │
│  │ ├─ Source-aware "Cap & Fill" balancing                                  │ │
│  │ ├─ Semantic grouping by CL ID (not text labels)                         │ │
│  │ └─ "CD4+ T cells" and "CD4-positive T cell" → same group                │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 5: train_celltypist_model()                                       │ │
│  │ ├─ SGD with mini-batch training                                         │ │
│  │ ├─ balance_cell_type=True for class weighting                           │ │
│  │ └─ Save .pkl model + metadata JSON + colors JSON                        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 6: annotate_celltypist() (wraps celltypist)                       │ │
│  │ ├─ Apply model to spatial data                                          │ │
│  │ ├─ Z-score confidence transformation                                    │ │
│  │ └─ majority_voting=False (per-cell predictions)                         │ │
│  │                                                                          │ │
│  │ OUTPUT: .obs["cell_type"], .obs["cell_type_confidence"]                 │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
...
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ STAGE 8: add_ontology_ids() (to predictions)                            │ │
│  │ Map final cell_type predictions to CL IDs                               │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                      │
│                                       ▼                                      │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
...
adata = train_and_annotate_config(
    adata,
    config,
    plot_output="./qc/"
)
...
```
FOR EACH REFERENCE:
├─ resolve_uri_to_local() - Download cloud files if needed
├─ load_adata_backed() - Memory-efficient loading
├─ validate_cell_type_column() - Check for issues
├─ normalize_gene_names() - Ensembl → HUGO
├─ ensure_normalized() - log1p(10k)
├─ Set .obs["original_label"] = label column
└─ Set .obs["reference_source"] = filename stem

GENE INTERSECTION:
├─ Subset to target_genes (panel genes)
└─ Find shared genes across all references

CONCATENATE:
└─ sc.concat(adatas, join="inner")

POST-CONCATENATION FILTERING:
├─ Remove excluded labels (exact match)
└─ Remove low-count cell types (if filter_min_cells=True)
```
...
PROPORTIONAL BALANCE:
  ...
  ✅ Both studies contribute proportionally
...
**Cap & Fill with target_proportions:**

For cell types in `target_proportions`:
- **Cap**: `proportion × total_input_cells` (e.g., 0.0025 × 25000 = 62 cells)
- **Fill**: But ensure at least `min_cells_per_type` for training quality

For cell types NOT in `target_proportions`:
- Normal `max_cells_per_type` capping applies
...
The pattern matching in `patterns.py` uses fall-through logic. Specific subtypes (like "cDC1" or "cDC2") match explicit patterns, but generic labels (like "conventional dendritic cell" without a number suffix) fall through to the parent category:
...
2. **Confidence over precision** — A high-confidence "dendritic cell" call is more useful than a low-confidence subtype call. Users can refine subtypes using spatial context (e.g., DCs in lymphoid aggregates vs tissue-resident).

3. **Avoiding false specificity** — Training on fine-grained labels when the model cannot distinguish them creates misleading predictions. The model learns spurious correlations instead of true biological signal.

4. **Downstream utility** — Most spatial analyses (niche identification, cell-cell interactions, domain detection) work well with coarser cell types. Subtype precision is rarely needed for these applications.

**When to keep fine-grained labels:**

- Panel includes subtype-discriminating markers (e.g., CLEC9A for cDC1)
- Specific biological question requires subtype resolution
- Validation confirms subtype calls are reliable

The mappings in `patterns.py` are transparent and can be customized for specific use cases where finer granularity is needed.
...
│                                                                              │
│  ┌───────────────────────────┐  ┌───────────────────────────┐               │
│  │  1. DEG Heatmap           │  │  2. 2D Marker Validation  │               │
│  │  ────────────────         │  │  ─────────────────────    │               │
│  │  • Top N genes per type   │  │  • GMM-3 trimodal         │               │
│  │  • Z-score normalized     │  │  • Faceted by cell type   │               │
│  │  • Row color annotations  │  │  • Red/Yellow/Green       │               │
│  │  • Cell type legend       │  │    thresholding           │               │
│  └───────────────────────────┘  └───────────────────────────┘               │
│                                                                              │
│  ┌───────────────────────────┐  ┌───────────────────────────┐               │
│  │  3. Confidence Plot       │  │  4. Ontology Mapping      │               │
│  │  ─────────────────        │  │  ────────────────────     │               │
│  │  • Left: Spatial heatmap  │  │  • Original → CL ID       │               │
│  │    (RdYlGn colormap)      │  │  • Match tier colors      │               │
│  │  • Right: Jitter plot     │  │  • Coverage statistics    │               │
│  │    (threshold line)       │  │  • Score per mapping      │               │
│  └───────────────────────────┘  └───────────────────────────┘               │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
**Warning:** `unsafe_force=True` may produce **incorrect results** if:
...
```
============================================================
UNSAFE NORMALIZATION FORCED
Detected X state: log1p_cpm
Estimated target_sum: 1000000.0
This may produce INCORRECT downstream results.
...
The normalization detection is validated with 42 tests covering:

| Test Category | What's Tested |
|---------------|---------------|
| Integer detection | Exact integers, float precision (1e-10 errors), mixed values |
| Raw count detection | Detection in X, rejection of log-transformed data, negative values |
| Target sum estimation | log1p_10k vs log1p_cpm via expm1 reversal |
| Layer search | Priority order, multiple layers, adata.raw.X |
| `ensure_normalized()` | All paths (raw→normalize, log1p_10k→no-op, error states) |
| `unsafe_force` | Allows normalization of invalid states with warning |
| Edge cases | Sparse matrices, single cell, float32, all-zero cells |
| Integration | Roundtrip normalization, scanpy compatibility |
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  ONTOLOGY MAPPING: 4-TIER SYSTEM                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  INPUT: "CD4+ T cells"                                                       │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ TIER 0: Pattern Canonicalization (patterns.py)                          │ │
│  │ ──────────────────────────────────────────────                          │ │
│  │ Regex patterns transform common variations to CL term names:            │ │
│  │                                                                          │ │
│  │   "CD4+ T cells" → "cd4-positive, alpha-beta t cell"                    │ │
│  │   "NK cells"     → "natural killer cell"                                │ │
│  │   "Macrophages"  → "macrophage"                                         │ │
│  │                                                                          │ │
│  │ Score: 0.95 (high confidence pattern match)                             │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                              │                                               │
│                              ▼                                               │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │ TIER 1: Exact Match (ontology_index.json)                               │ │
│  │ ─────────────────────────────────────────                               │ │
│  │ Direct lookup in pre-built CL index:                                    │ │
│  │                                                                          │ │
│  │   "cd4-positive, alpha-beta t cell" → CL:0000624                        │ │
│  │                                                                          │ │
│  │ Score: 1.0 (exact match) or 0.95 (via pattern)                          │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                              │                                               │
│                              ▼ (if no exact match)                           │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
...
│  │ ────────────────────────                                                │ │
│  │ Extracts biological tokens (CD markers, core words) and matches:        │ │
│  │                                                                          │ │
│  │   "CD8+ cytotoxic T" → tokens: [cd8, cytotoxic, t]                      │ │
│  │   Match: "cd8-positive, alpha-beta t cell"                              │ │
│  │                                                                          │ │
│  │ Score: 0.60-0.85                                                        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                              │                                               │
...
│  │ ─────────────────────────────────────────                               │ │
│  │ Last resort: word overlap between label and CL terms                    │ │
│  │                                                                          │ │
│  │ Score: 0.50-0.70                                                        │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                              │                                               │
│                              ▼                                               │
│  OUTPUT: CL:0000624, "cd4-positive, alpha-beta t cell", score=0.95          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
"cl": {
  ...existing terms...
  "my custom cell type": {"id": "CL:XXXXXXX", "name": "my custom cell type"}
}
```

3. The key (e.g., `"my custom cell type"`) is what gets matched against canonicalized labels
4. Look up valid CL IDs at: https://www.ebi.ac.uk/ols/ontologies/cl

### Customizing Pattern Matching

The `patterns.py` file contains regex patterns that canonicalize common label variations **before** ontology lookup. This is Tier 0 matching.
...
**Example 1: Adding an abbreviation**
...
- `|` = OR operator
- This maps both "macrophages" and standalone "Mph" to "macrophage"

**Example 2: Handling tissue-specific naming**

To map "Club (nasal)" and "Club cell" to the generic club cell:

```python
# In patterns.py, Lung/Airway section:
r"\bclub\b|clara": "club cell",
```

- `\bclub\b` = "club" as a standalone word (matches "Club (nasal)", "Club cell", etc.)
- `clara` = legacy name for club cells
- Both map to "club cell" which exists in CL

**Example 3: Handling cell type with modifiers**

To map "Migratory DCs" to the specific migratory dendritic cell type:

```python
# In patterns.py, Dendritic cells section:
r"migratory.*dc|migratory.*dendritic": "migratory dendritic cell",
```

- `.*` = any characters between words
- This catches "Migratory DCs", "migratory dendritic cells", etc.
...
print(get_canonical_term("Alveolar Mph"))      # Should return: "alveolar macrophage"
```
...
The saved `*_ontology_mapping_metadata.json` file contains detailed information about each mapping, including closest matches for unmapped terms.
...
- Automatically used by `generate_annotation_plots()` in the `train_and_annotate()` pipeline for 2D marker validation plots
- Can be loaded directly for quick marker-based investigation:
...
**Note:** These markers are curated from literature and public atlases. They are not perfect and may not cover all cell types in your data. Use as a starting point, not ground truth.
